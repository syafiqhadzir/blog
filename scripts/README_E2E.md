...
- **Nightly Workflow**: Runs `npm run test:full` on Chromium, Firefox, WebKit.
  Artifacts uploaded on failure.

## Troubleshooting

- **Server Start**: Note that `npm run test:*` commands invoke `http-server` via
  Playwright's `webServer` config.
- **Build**: The site is rebuilt (`bundle exec jekyll build`) automatically
  before tests start suitable for `CI`.
