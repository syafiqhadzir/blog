...
async function auditSEO() {
  ...
  if (!fs.existsSync(siteDir)) {
    process.stderr.write(
      'Error: _site directory not found. Run jekyll build first.\n',
    );
    process.exit(1);
  }
  ...
  for (const file of files) {
    ...
    if (!canonical) {
      process.stderr.write(
        `[ERROR] ${relativePath}: Missing rel="canonical" tag\n`,
      );
      errors++;
    }
    ...
    if (!description || !description.getAttribute('content')) {
      process.stdout.write(
        `[WARN] ${relativePath}: Missing meta description\n`,
      );
      warnings++;
    }
    ...
    if (h1s.length > 1) {
      process.stdout.write(
        `[WARN] ${relativePath}: Multiple H1 tags found (${h1s.length})\n`,
      );
      warnings++;
    ...
    for (const img of images) {
      if (!img.getAttribute('alt')) {
        ...
        process.stdout.write(
          `[WARN] ${relativePath}: Missing alt text on ${img.tagName} (src: ${src})\n`,
        );
        warnings++;
      }
    }
    ...
    if (!schema) {
      process.stdout.write(
        `[WARN] ${relativePath}: Missing JSON-LD structured data\n`,
      );
      warnings++;
    }
  }
  ...
  process.stdout.write(
    `Warnings: ${String(warnings)} (Best practice recommendations)\n`,
  );
