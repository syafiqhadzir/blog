...
This is not engineering. This is guessing. Root Cause Analysis (RCA) is the art of finding the *one true reason* things broke, so you can kill it forever.
...
- **Changes correlate with issues**: Correlation does not equal causation, but it is a good place to start. "What changed at 14:01?"
...
**QA's job is to reject the "Fix" if it does not address the Root Cause**. If a dev says "I restarted it and it works now", fail the ticket.

## Fishbone Diagrams (Ishikawa)

Draw a fish skeleton. Head = The Problem (e.g., "Latency Spike"). Ribs = Categories (People, Process, Technology, Environment).
...
But a Blameless RCA culture allows for growth. If you punish mistakes, people hide the root cause ("It was a glitch"). If you celebrate finding the root cause, you build resilience.

## Key Takeaways

- **Blameless culture enables learning**: Focus on the *system* that allowed the human error, not the human. Why was it possible for Dave to push to Prod on Friday? Where was the guardrail?
- **Swiss Cheese Model explains failures**: Accidents happen when holes in multiple layers of defence align. Close the holes.
