...
- **Low-Level control demands responsibility**: You handle chunks, keyframes, and timestamps directly.
- **Zero Copy is the goal**: Keep frames on the GPU. Reading a frame to CPU (`readPixels`) is slow.
- **Latency is measured in microseconds**: Not seconds. Not milliseconds. Microseconds.
...
- **Learn**: Understand **I-Frames** (Keyframes), **P-Frames** (Predicted), and **B-Frames** (Bi-directional).
- **Experiment**: Build a simple screen recorder implementation using `getDisplayMedia` + WebCodecs.
