...
If a payment API processes a retry as a second charge, you have just stolen money. This is bad.

**Idempotency** is the mathematical property where `f(x) = f(f(x))`. Doing it once is the same as doing it twice.

## TL;DR

- **Safe Methods are naturally idempotent**: `GET`, `PUT`, `DELETE` should naturally be idempotent.
- **Unsafe Methods need help**: `POST` is NOT idempotent by default. It needs help.
- **The Key enables deduplication**: Use a unique `Idempotency-Key` header to deduplicate requests.

## The Double Pizza Problem

Imagine you are ordering a pizza. You click "Buy". The loading spinner spins. Nothing happens. You click "Buy" again.

Two hours later, two pizzas arrive. You are happy (more pizza), but your bank account is sad.
...
2. **Call API** with Key `A` (again). Assert 201 Created (or 200 OK). Check DB count = 1.
