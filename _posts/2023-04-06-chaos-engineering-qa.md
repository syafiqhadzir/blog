...
- **Hypothesis drives experiments**: "If I kill one server, the load balancer should redirect traffic instantly."
- **Blast Radius needs limiting**: Start small (kill one pod), then go big (kill an Availability Zone).
- **Observability is mandatory**: If you cannot measure the pain, do not inflict it.
...
Run this in your Staging environment. Watch your monitoring dashboard. Do you get an alert? If not, fix your alerts.
...
- **Automate chaos**: Chaos should run continuously (like Netflix's Chaos Monkey).
- **Fix It**: If an experiment fails, it is a P1 bug. Fix it before running it again.

## Next Steps

- **Game Day**: Schedule a "Game Day" where the team gathers to run manual chaos experiments.
- **Kill a Pod**: Go to Kubernetes and delete a random pod. See what happens.
- **Add Latency**: Use your API Gateway to inject 2s latency. Does the frontend show a spinner or a blank screen?
