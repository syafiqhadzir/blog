...
In HTTP land, if you have 10,000 users, you might only have 100 active requests at any specific millisecond.

In WebSocket land, 10,000 users means 10,000 open TCP connections. This consumes:

1. **File Descriptors**: Creating a "Too many open files" error on Linux properly configured servers.
...
Here is a K6 script that connects to a chat server, subscribes to a channel, sends a message, and expects an echo.
...
- **Monitor**: Connect a Grafana dashboard to K6 to visualise the latency spikes.
- **Crash It**: Find the exact number of connections that makes your server run out of RAM.
