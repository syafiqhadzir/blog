...
QAs do not just test the code anymore; we test the *switch*.

## TL;DR

- **Concept**: Deploy to an idle environment ("Green"). Test it. Flip the Load Balancer. Zero Downtime.
- **Risk**: Database compatibility. Both Blue and Green share the *same* database.
- **Validation**: Smoke test the Green environment using a private URL or header (`X-Route-To: green`).

## The Pre-Switch Panic

The beauty of Blue-Green is that you can test "Production" without actually being *in* Production.
...
3. ...Wait...
4. Delete old column (in the *next* deployment).
