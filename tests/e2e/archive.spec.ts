...
test.describe('Archive & Search', () => {
  ...
  test('should display search input and popular topics', async ({ page }) => {
    ...
    await expect(page.locator('.suggestions-label')).toContainText(
      'Popular topics',
    );
  });
  ...
  test('should handle case-insensitive selection from autocomplete', async ({
    page,
  }) => {
    const searchInput = page.locator('#archive-search-input');
  ...
  test('should show no results message for non-matching queries', async ({
    page,
  }) => {
    const searchInput = page.locator('#archive-search-input');
  ...
  test('should restore popular topics when search is cleared', async ({
    page,
  }) => {
    const searchInput = page.locator('#archive-search-input');
  ...
  test('should toggle year groups correctly via Load More', async ({
    page,
  }) => {
    const loadMore = page.locator('.load-more-years');

    // Initially 3 years
    await expect(page.locator('details.year-group:not([hidden])')).toHaveCount(
      3,
    );
